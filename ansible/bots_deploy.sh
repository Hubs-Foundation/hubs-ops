...
ansible-playbook -i "${TARGET_IPS}," --extra-vars "bot_type=${BOT_TYPE} env=${ENVIRONMENT}" --ssh-common-args="-i ~/.ssh/mozilla_mr_id_rsa -o ProxyCommand=\"ssh -W %h:%p -o ControlMaster=auto -o ControlPersist=120s -o ControlPath=\"~/.ssh/%r@%h:%p\" -o ConnectTimeout=90 -o StrictHostKeyChecking=no ubuntu@${BASTION_IP}\"" -u ubuntu "bots.yml"
