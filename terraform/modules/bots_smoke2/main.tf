...
resource "aws_security_group" "bots_smoke2" {
  name = "${var.shared["env"]}-bots_smoke2"
  count = "${var.enabled}"
...
resource "aws_iam_role" "bots_smoke2" {
  name = "${var.shared["env"]}-bots_smoke2"
  count = "${var.enabled}"
...
resource "aws_iam_role_policy_attachment" "bots_smoke2-base-policy" {
  count = "${var.enabled}"

  role = "${aws_iam_role.bots_smoke2.name}"
  policy_arn = "${data.terraform_remote_state.base.base_policy_arn}"
}

resource "aws_iam_instance_profile" "bots_smoke2" {
  name = "${var.shared["env"]}-bots_smoke2"
  count = "${var.enabled}"

  role = "${aws_iam_role.bots_smoke2.id}"
}

resource "aws_launch_configuration" "bots_smoke2" {
  count = "${var.enabled}"
  image_id = "${data.aws_ami.squawk-ami.id}"
  instance_type = "${var.bots_smoke2_instance_type}"
  security_groups = ["${aws_security_group.bots_smoke2.id}"]
  key_name = "${data.terraform_remote_state.base.mr_ssh_key_id}"
  iam_instance_profile = "${aws_iam_instance_profile.bots_smoke2.id}"
  associate_public_ip_address = false
  lifecycle { create_before_destroy = true }
}

resource "aws_autoscaling_group" "bots_smoke2" {
  name = "${var.shared["env"]}-bots_smoke2"
  count = "${var.enabled}"

  launch_configuration = "${aws_launch_configuration.bots_smoke2.id}"
  availability_zones = ["${data.aws_availability_zones.all.names}"]
  vpc_zone_identifier = ["${data.terraform_remote_state.vpc.private_subnet_ids}"]

  min_size = "${var.min_bots_smoke2_servers}"
  max_size = "${var.max_bots_smoke2_servers}"

  lifecycle { create_before_destroy = true }
  tag { key = "env", value = "${var.shared["env"]}", propagate_at_launch = true }
  tag { key = "host-type", value = "${var.shared["env"]}-bots_smoke2", propagate_at_launch = true }
}
